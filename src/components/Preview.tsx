...
import { METHOD_COLORS, STATUS_COLORS, generateMarkdown, generateCurl } from '@/lib/utils'
...
function CopyButton({ text, label = 'Copy' }: { text: string; label?: string }) {
  ...
  return (
    <Button variant="ghost" size="sm" onClick={handleCopy} className={cn('gap-1.5 text-xs', copied && 'text-emerald-500')}>
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-3 pt-1">{children}</h3>
  )
}
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} className="border-b border-border/50 last:border-0 hover:bg-muted/30 transition-colors">
              {row.map((cell, j) => (
                <td key={j} className="px-3 py-2.5 text-sm">{cell}</td>
              ))}
            </tr>
          ))}
        </tbody>
    ...
    params.length > 0 && (
      <div>
        ...
        <Table
          ...
          rows={params.map(p => [
            <code className="text-xs font-mono text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/50 px-1.5 py-0.5 rounded">{p.name}</code>,
            <code className="text-xs font-mono text-amber-600 dark:text-amber-400">{p.type}</code>,
            p.required ? <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-400">Yes</span> : <span className="text-xs text-muted-foreground">—</span>,
            p.defaultValue ? <code className="text-xs font-mono text-muted-foreground">{p.defaultValue}</code> : <span className="text-muted-foreground">—</span>,
    ...
    <div className="p-6 max-w-none space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-3 flex-wrap">
          <h1 className="font-display text-2xl font-bold text-foreground">{ep.name}</h1>
        </div>
        ...
        {ep.deprecated && (
          <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 mb-3">
            ...
            <p className="text-sm text-amber-700 dark:text-amber-400">
              <strong>Deprecated.</strong> {ep.deprecatedMessage || 'This endpoint is deprecated.'}
            </p>
          </div>
        )}
        ...
        <div className="space-y-2">
          {environments.map((env) => {
            ...
            return (
              <div key={env} className="inline-flex items-center rounded-xl border border-border overflow-hidden shadow-sm mr-2">
                <span className={cn('px-3 py-2 text-xs font-bold border-r border-border', STATUS_COLORS[env])}>
                  {env}
                </span>
                <span className={cn('px-3 py-2 text-xs font-bold font-mono tracking-widest border-r border-border', METHOD_COLORS[ep.method])}>
                  {ep.method}
                </span>
        ...
        {ep.description && (
          <p className="mt-3 text-muted-foreground text-sm leading-relaxed">{ep.description}</p>
        )}
      </div>
        ...
        <div>
          ...
          <Table
            ...
            rows={ep.headers.map(h => [
              ...
              <code className="text-xs font-mono text-muted-foreground">{h.value}</code>,
              h.required ? <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-400">Yes</span> : <span className="text-xs text-muted-foreground">—</span>,
      ...
      {ep.isEncrypted && ep.decryptedRequestBody && (
        <div>
          <SectionTitle>Decrypted Request Body</SectionTitle>
        ...
        <div>
          ...
          <Table
            ...
            rows={ep.errorCodes.map(e => [
              <code className="text-xs font-mono text-red-500 dark:text-red-400 font-semibold">{e.code}</code>,
...
export function Preview({ endpoint, mode }: PreviewProps) {
  ...
  const content = mode === 'markdown' ? generateMarkdown(endpoint) : generateCurl(endpoint)
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-3">
        ...
        <CopyButton text={content} label={`Copy ${mode === 'markdown' ? 'Markdown' : 'cURL'}`} />
      </div>
