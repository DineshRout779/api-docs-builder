...
export const METHOD_COLORS: Record<HttpMethod, string> = {
  ...
  PATCH:
    'text-violet-600 bg-violet-50 border-violet-200 dark:text-violet-400 dark:bg-violet-950/50 dark:border-violet-800',
  DELETE:
...
function inferEnvironmentFromHost(host: string): EnvironmentType {
  ...
  if (h.includes('prod') || h.startsWith('api.') || h.includes('production'))
    return 'Production';
  return 'Staging';
}
...
export function parseJsonToParams(
  jsonStr: string,
    ...
    const processObject = (obj: any) => {
      Object.entries(obj).forEach(([key, value]) => {
        params.push({
          id: uid(),
          name: key,
          type: inferParamType(String(value)),
          location,
          required: false,
          description: '',
          example:
...
export function parseCurlToEndpoint(
  curl: string,
  current: ApiEndpoint,
): Partial<ApiEndpoint> {
...
export function generateMarkdown(ep: ApiEndpoint): string {
  ...
  const errorRows = ep.errorCodes
    .map((e) => `| \`${e.code}\` | ${e.description} |`)
    .join('\n');

  const decryptedParams = ep.params.filter((p) => p.location === 'decrypted');
  const decryptedParamRows = decryptedParams
    .map(
      (p) =>
        `| \`${p.name}\` | \`${p.type}\` | ${p.required ? 'Yes' : '-'} | ${p.defaultValue ? `\`${p.defaultValue}\`` : '-'} | ${p.description}${p.example ? ` (e.g. \`${p.example}\`)` : ''} |`,
    )
    .join('\n');
...
${
  ep.isEncrypted && (ep.decryptedRequestBody || decryptedParams.length > 0)
    ? `## Decrypted Request Information
...
export async function generateDocx(ep: ApiEndpoint): Promise<Blob> {
  ...
  children.push(
    new Paragraph({
      children: [
        new TextRun({
          text: `${ep.method} ${envConfig.baseUrl}${ep.path}`,
          bold: true,
        }),
      ],
      spacing: { after: 160 },
  ...
  children.push(dataTable(['Environment', 'URL', 'API Type'], envRows));
  ...
  if (ep.headers.length) {
    ...
    children.push(
      dataTable(
        ['Header', 'Value', 'Required', 'Description'],
        ep.headers.map((h: ApiHeader) => [
          h.key,
          h.value,
          boolText(h.required),
          textOrDash(h.description),
        ]),
      ),
    );
  }
  ...
  const decryptedParams = ep.params.filter(
    (p: ApiParam) => p.location === 'decrypted',
  );
  if (ep.isEncrypted && (ep.decryptedRequestBody || decryptedParams.length)) {
    children.push(sectionTitle('Decrypted Request Information'));
    if (ep.decryptedRequestBody) {
      children.push(
        new Paragraph({
          children: [
            new TextRun({ text: 'Decrypted Body Example', bold: true }),
          ],
          spacing: { after: 100 },
        }),
      );
    ...
    if (decryptedParams.length) {
      children.push(
        new Paragraph({
          children: [new TextRun({ text: 'Decrypted Fields', bold: true })],
      ...
      children.push(
        dataTable(
          ['Name', 'Type', 'Required', 'Default', 'Description'],
          decryptedParams.map((p: ApiParam) => [
            p.name,
            p.type,
            boolText(p.required),
            textOrDash(p.defaultValue),
  ...
  children.push(
    new Paragraph({
      children: [
        ...
        new TextRun(ep.responseStatusCode),
      ],
      spacing: { after: 100 },
    }),
  );
  ...
  children.push(
    new Paragraph({
      children: [
        new TextRun({
          text: 'Generated with API Docs Builder',
          italics: true,
          color: '6B7280',
        }),
      ],
